        ...
        public void AddCommandToAction(int actionId, int codeBlockId)
        {
            ...
            if (actionCodeBlockLocation == 0 || (actionCodeBlockLocation >= openAreaStartLocation * 4 && actionCodeBlockLocation < PsaFile.DataSectionSize))
            {
                ...
                if (actionCodeBlockLocation == 0)
                {
                    ...
                    for (stoppingPoint = openAreaStartLocation; stoppingPoint < dataSectionSizeBytes; stoppingPoint++)
                    {
                        if (PsaFile.FileContent[stoppingPoint] == -86052851) // -86052851 is equal to FFFF FFFF FADE F00D in hex, which means empty space that PSA created...I think
                        {
                            for (bitStoppingPoint = stoppingPoint + 1; bitStoppingPoint <= stoppingPoint + 3; bitStoppingPoint++)
                            {
                                if (PsaFile.FileContent[bitStoppingPoint] != -86052851)
                                {
                                    stoppingPoint = bitStoppingPoint;
                                    break;
                                }
                            }
                            if (bitStoppingPoint == stoppingPoint + 4)
                            {
                                break;
                            }
                        }
                    ...
                    PsaFile.FileContent[codeBlockLocation + actionId] = stoppingPoint * 4;
                    PsaFile.PointerInterlockTracker[PsaFile.NumberOfOffsetEntries] = (codeBlockLocation + actionId) * 4;
                ...
                else
                {
                    ...
                    // I think this part checks if there is room for existing command, and if there is, it doesn't need to move the existing commands all over to a new location so it dips early and just adds the nop
                    ...
                    {
                        PsaFile.FileContent[commandStartLocation + numberOfCommandsAlreadyInAction * 2] = 131072; // 131072 is nop
                        ...
                        newOffsetLocation = actionCodeBlockLocation / 4;
            ...
            Console.WriteLine("NEW OFFSET LOCATION: " + newOffsetLocation);
        }
